...
#ifdef GC_PTHREADS
#if defined(DARWIN) || (defined(GC_WIN32_THREADS) && defined(EMULATE_PTHREAD_SEMAPHORE))
...
#include <sys/time.h>
#endif
#if defined(GC_EXPLICIT_SIGNALS_UNBLOCK) || !defined(GC_NO_PTHREAD_SIGMASK) || (defined(GC_PTHREADS_PARAMARK) && !defined(NO_MARKER_SPECIAL_SIGMASK))
#include <signal.h>
#endif
...
#if defined(GC_PTHREADS) && !defined(PLATFORM_THREADS) && !defined(SN_TARGET_PSP2)
...
#if defined(OSF1) && defined(_PTHREAD_USE_MANGLED_NAMES_) && !defined(_PTHREAD_USE_PTDNAM_)
...
#if !defined(GC_WIN32_THREADS) && !defined(PLATFORM_THREADS) && !defined(SN_TARGET_PSP2)
...
GC_API int
        GC_pthread_create(pthread_t* t, GC_PTHREAD_CREATE_CONST pthread_attr_t* a, void* (*fn)(void*), void* arg) {
...
#endif
    REAL_FUNC(pthread_create) = TYPED_DLSYM(GC_pthread_create_t, dl_handle, "pthread_create");
#ifdef RTLD_NEXT
...
#endif
#ifndef GC_NO_PTHREAD_SIGMASK
    REAL_FUNC(pthread_sigmask) = TYPED_DLSYM(GC_pthread_sigmask_t, dl_handle, "pthread_sigmask");
#endif
    REAL_FUNC(pthread_join) = TYPED_DLSYM(GC_pthread_join_t, dl_handle, "pthread_join");
    REAL_FUNC(pthread_detach) = TYPED_DLSYM(GC_pthread_detach_t, dl_handle, "pthread_detach");
#ifndef GC_NO_PTHREAD_CANCEL
    REAL_FUNC(pthread_cancel) = TYPED_DLSYM(GC_pthread_cancel_t, dl_handle, "pthread_cancel");
#endif
#ifdef GC_HAVE_PTHREAD_EXIT
    REAL_FUNC(pthread_exit) = TYPED_DLSYM(GC_pthread_exit_t, dl_handle, "pthread_exit");
#endif
...
#if defined(GC_WIN32_THREADS) || defined(USE_PROC_FOR_LIBRARIES) || (defined(IA64) && (defined(HAVE_PTHREAD_ATTR_GET_NP) || defined(HAVE_PTHREAD_GETATTR_NP)))
...
#elif defined(HAVE_PTHREAD_SETNAME_NP_WITH_TID) || defined(HAVE_PTHREAD_SETNAME_NP_WITHOUT_TID) || defined(HAVE_PTHREAD_SET_NAME_NP)
...
/* defined.                                                         */
static pthread_mutex_t mark_mutex = { 0, 0, 0, PTHREAD_MUTEX_ERRORCHECK_NP, { 0, 0 } };
#else
...
GC_INNER void
    ...
    if(sigdelset(&set, GC_get_suspend_signal()) != 0 || sigdelset(&set, GC_get_thr_restart_signal()) != 0)
...
GC_INNER_WIN32THREAD GC_thread
    ...
    if(EXPECT(NULL == first_thread.crtn, FALSE)) {
...
#if defined(CPPCHECK) && defined(THREAD_SANITIZER) && defined(SIGNAL_BASED_STOP_WORLD)
        ...
        saved_crtn = crtn;
        result = (GC_thread)GC_INTERNAL_MALLOC(sizeof(struct GC_Thread_Rep), NORMAL);
...
GC_INNER_WIN32THREAD void
    ...
    /* else */ {
        ...
        GC_ASSERT(I_HOLD_LOCK());
#if defined(DEBUG_THREADS) && !defined(MSWINCE) && (!defined(MSWIN32) || defined(CONSOLE_LOG))
...
GC_API void GC_CALL
        GC_register_altstack(void* normstack, size_t normstack_size, void* altstack, size_t altstack_size) {
#ifdef GC_WIN32_THREADS
    /* TODO: Implement */
...
#elif defined(AIX) || defined(COSMO) || defined(HAIKU) || defined(HOST_ANDROID) || defined(HURD) || defined(NACL) || defined(OSF1) || defined(SOLARIS)
GC_INLINE int
        GC_get_nprocs(void) {
...
#define PROC_STAT_BUF_SZ ((1 + MAX_MARKERS) * 100) /* should be enough */
    ...
    for(i = 0; i < len - 4; ++i) {
        if(stat_buf[i] == '\n' && stat_buf[i + 1] == 'c' && stat_buf[i + 2] == 'p' && stat_buf[i + 3] == 'u') {
...
#endif
            LOCK();
        } while(GC_incremental && collection_in_progress() && (wait_for_all || old_gc_no == GC_gc_no));
    }
#endif
}
...
#ifdef PARALLEL_MARK
    if(GC_parallel) {
#if defined(THREAD_SANITIZER) && defined(GC_ASSERTIONS) && defined(CAN_CALL_ATFORK)
...
static void
    ...
    if(GC_parallel) {
        ...
        GC_release_mark_lock();
#else
#if !defined(GC_ASSERTIONS) || (defined(THREAD_SANITIZER) && defined(CAN_CALL_ATFORK))
...
GC_INNER_WIN32THREAD void
    ...
    if(GC_handle_fork) {
#ifdef CAN_CALL_ATFORK
        if(pthread_atfork(fork_prepare_proc, fork_parent_proc, fork_child_proc) == 0) {
...
#if !defined(GC_NO_THREADS_DISCOVERY) && defined(GC_WIN32_THREADS) || !defined(DONT_USE_ATEXIT)
    ...
    /* locks cannot be used inside signal handlers.             */
    {
        ...
        if(cpu_set_cnt > 1) {
            ...
            if(cpu_lowest_set < cpu_highest_set) {
                ...
                cpu_lowest_set += (unsigned)getpid() % (cpu_highest_set - cpu_lowest_set + 1);
            }
#endif
            CPU_ZERO(&mask);
            /* Select just one CPU. */
...
GC_API void GC_CALL
    ...
    crtn = t->crtn;
    GC_ASSERT((t->flags & DO_BLOCKING) == 0 && NULL == crtn->traced_stack_sect); /* for now */
...
#if !defined(GC_NO_THREADS_DISCOVERY) && defined(GC_WIN32_THREADS)
    GC_ASSERT(!GC_win32_dll_threads || THREAD_ID_EQUAL(GC_main_thread_id, thread_id_self()));
#endif
...
#if defined(PTHREAD_STOP_WORLD_IMPL) && !defined(NO_SIGNALS_UNBLOCK_IN_MAIN) || defined(GC_EXPLICIT_SIGNALS_UNBLOCK)
...
GC_INNER_PTHRSTART GC_thread
        GC_start_rtn_prepare_thread(void* (**pstart)(void*), void** pstart_arg, struct GC_stack_base* sb, void* arg) {
...
#if ((defined(GC_PTHREADS_PARAMARK) || defined(USE_PTHREAD_LOCKS)) && !defined(NO_PTHREAD_TRYLOCK)) || defined(USE_SPIN_LOCK)
...
#if (!defined(USE_SPIN_LOCK) && !defined(NO_PTHREAD_TRYLOCK) && defined(USE_PTHREAD_LOCKS)) || defined(GC_PTHREADS_PARAMARK)
...
#if defined(GC_ASSERTIONS) && defined(GC_WIN32_THREADS) && !defined(USE_PTHREAD_LOCKS)
