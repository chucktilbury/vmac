...
#if !defined(AO_DISABLE_GCC_ATOMICS) && ((defined(__clang__) && __clang_major__ >= 8) /* clang 8.0+ */ \
...
int CORD_ncmp(CORD x, size_t x_start, CORD y, size_t y_start, size_t len) {
    ...
    for(count = 0; count < len;) {
        ...
        if(!CORD_pos_valid(ypos))
            return 1;
        if((avail = CORD_pos_chars_left(xpos)) <= 0 || (yavail = CORD_pos_chars_left(ypos)) <= 0) {
            char xcurrent = CORD_pos_fetch(xpos);
            char ycurrent = CORD_pos_fetch(ypos);
...
size_t
    ...
    for(match_pos = start;; match_pos++) {
        if((x_buf & mask) == s_buf && (slen == start_len || CORD_ncmp(x, match_pos + start_len, s, start_len, slen - start_len) == 0))
            break;
...
static void* GC_CALLBACK
    ...
    if(line_start != state->lf_current && fseek(f, (long)line_start, SEEK_SET) != 0) {
        ABORT("fseek failed");
    }
    if(fread(new_cache->data, sizeof(char), LINE_SZ, f) <= file_pos - line_start) {
        ABORT("fread failed");
    }
...
static char
    ...
    cache_line* volatile* cl_addr = &state->lf_cache[DIV_LINE_SZ(MOD_CACHE_SZ(i))];
    cache_line* cl;
...
CORD CORD_from_file_lazy(FILE* f) {
    ...
    if(fseek(f, 0l, SEEK_END) != 0 || (len = ftell(f)) < 0 || fseek(f, 0l, SEEK_SET) != 0) {
        ABORT("Bad f argument or I/O failure");
    }
