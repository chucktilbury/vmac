...
STATIC GC_bool
    ...
    result = (struct hblk**)GC_scratch_alloc((MAXOBJGRANULES + 1) * sizeof(struct hblk*));
...
GC_INNER void*
    ...
    if(SMALL_OBJ(lb) && EXPECT(align_m1 < HBLKSIZE / 2, TRUE)) {
        ...
        if(EXPECT(op != NULL, TRUE)) {
            GC_ASSERT(PTRFREE == k || NULL == obj_link(op) || (ADDR(obj_link(op)) < GC_greatest_real_heap_addr && GC_least_real_heap_addr < ADDR(obj_link(op))));
            *opp = obj_link(op);
            if(k != PTRFREE)
                obj_link(op) = NULL;
            GC_bytes_allocd += GRANULES_TO_BYTES((word)lg);
...
void free(void* p) {
    ...
    UNUSED_ARG(p);
#else
#if defined(REDIR_MALLOC_AND_LINUXTHREADS) && !defined(USE_PROC_FOR_LIBRARIES)
