...
STATIC struct GC_ms_entry* GC_CALLBACK
        GC_gcj_fake_mark_proc(word* addr, struct GC_ms_entry* mark_stack_top, struct GC_ms_entry* mark_stack_limit, word env) {
    UNUSED_ARG(addr);
...
#define ignore_gcj_info TRUE
    ...
    if(ignore_gcj_info) {
        ...
        GC_gcj_kind = (int)GC_new_kind_inner(
                ...
                (((word)(-(GC_signed_word)GC_GCJ_MARK_DESCR_OFFSET - GC_INDIR_PER_OBJ_BIAS)) | GC_DS_PER_OBJECT),
                FALSE, TRUE);
...
void* GC_core_gcj_malloc(size_t lb, const void* vtable_ptr, unsigned flags) {
    ...
    LOCK();
    if(SMALL_OBJ(lb) && (op = GC_gcjobjfreelist[lg = GC_size_map[lb]], EXPECT(op != NULL, TRUE))) {
