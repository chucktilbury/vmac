...
#define HASH3(addr, size, log_size) \
    ((size_t)((ADDR(addr) >> 3) ^ (ADDR(addr) >> (3 + (log_size)))) & ((size) - (size_t)1))
...
/* success.                                                             */
STATIC void
        GC_grow_table(struct hash_chain_entry*** table_ptr, unsigned* log_size_ptr, const size_t* entries_ptr) {
    size_t i;
...
STATIC int
    ...
    if(EXPECT(NULL == dl_hashtbl->head, FALSE) || EXPECT(dl_hashtbl->entries > ((size_t)1 << dl_hashtbl->log_size), FALSE)) {
...
STATIC int
        GC_move_disappearing_link_inner(struct dl_hashtbl_s* dl_hashtbl, void** link, void** new_link) {
...
GC_API int GC_CALL
    ...
    if((ADDR(new_link) & (ALIGNMENT - 1)) != 0 || !NONNULL_ARG_NOT_NULL(new_link))
...
STATIC void
        GC_register_finalizer_inner(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd, finalization_mark_proc mp) {
    ...
    if(EXPECT(NULL == GC_fnlz_roots.fo_head, FALSE) || EXPECT(GC_fo_entries > ((size_t)1 << GC_log_fo_table_size), FALSE)) {
...
GC_API void GC_CALL
        GC_register_finalizer(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
    GC_register_finalizer_inner(obj, fn, cd, ofn, ocd,
                                GC_normal_finalize_mark_proc);
}

GC_API void GC_CALL
        GC_register_finalizer_ignore_self(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
    GC_register_finalizer_inner(obj, fn, cd, ofn, ocd,
                                GC_ignore_self_finalize_mark_proc);
}

GC_API void GC_CALL
        GC_register_finalizer_no_order(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
    GC_register_finalizer_inner(obj, fn, cd, ofn, ocd,
                                GC_null_finalize_mark_proc);
}

GC_API void GC_CALL
        GC_register_finalizer_unreachable(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
    ...
    for(i = 0; i < dl_size; i++) {
        ...
        for(curr_dl = dl_hashtbl->head[i]; curr_dl != NULL; curr_dl = next_dl) {
            ...
            if(is_remove_dangling) {
                ptr_t real_link = (ptr_t)GC_base(GC_REVEAL_POINTER(curr_dl->dl_hidden_link));
    ...
    for(i = 0; i < fo_size; i++) {
        ...
        while(curr_fo != NULL) {
            ...
            if(!GC_is_marked(real_ptr)) {
                ...
                /* see it.                                                */
                curr_fo->fo_hidden_base = (GC_hidden_pointer)GC_REVEAL_POINTER(curr_fo->fo_hidden_base);

                GC_bytes_finalized += (word)curr_fo->fo_object_sz + sizeof(struct finalizable_object);
...
STATIC void
    ...
    for(i = 0; i < fo_size; i++) {
        ...
        while(curr_fo != NULL) {
            ...
            /* see it.                                                    */
            curr_fo->fo_hidden_base = (GC_hidden_pointer)GC_REVEAL_POINTER(curr_fo->fo_hidden_base);
            GC_bytes_finalized += curr_fo->fo_object_sz + sizeof(struct finalizable_object);
            curr_fo = next_fo;
        }
    }
