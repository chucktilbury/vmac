...
const GC_VERSION_VAL_T GC_version = ((GC_VERSION_VAL_T)GC_VERSION_MAJOR << 16) | (GC_VERSION_MINOR << 8) | GC_VERSION_MICRO;
#endif

GC_API GC_VERSION_VAL_T GC_CALL
        GC_get_version(void) {
    return ((GC_VERSION_VAL_T)GC_VERSION_MAJOR << 16) | (GC_VERSION_MINOR << 8) | GC_VERSION_MICRO;
}
...
STATIC word
    ...
    GC_signed_word expl_managed = (GC_signed_word)GC_non_gc_bytes - (GC_signed_word)GC_non_gc_bytes_at_gc;

    /* Don't count what was explicitly freed, or newly allocated for    */
    /* explicit management.  Note that deallocating an explicitly       */
    /* managed object should not alter result, assuming the client      */
    /* is playing by the rules.                                         */
    result = (GC_signed_word)GC_bytes_allocd + (GC_signed_word)GC_bytes_dropped - (GC_signed_word)GC_bytes_freed + (GC_signed_word)GC_finalizer_bytes_freed - expl_managed;
...
#define INITIAL_HEAP_SECTS 32
        ...
        void* new_heap_sects = GC_scratch_alloc(new_capacity * sizeof(struct HeapSect));

        if(NULL == new_heap_sects) {
            /* Retry with smaller yet sufficient capacity.  */
            new_capacity = GC_n_heap_sects + INITIAL_HEAP_SECTS;
            new_heap_sects = GC_scratch_alloc(new_capacity * sizeof(struct HeapSect));
            if(NULL == new_heap_sects)
    ...
    for(i = 0; i < GC_n_heap_sects; i++) {
        ...
        GC_ASSERT(!(ADDR_INSIDE((ptr_t)h, hs_start, hs_end) || (ADDR_LT(hs_start, endp) && ADDR_GE(hs_end, endp)) || (ADDR_LT((ptr_t)h, hs_start) && ADDR_LT(hs_end, endp))));
    }
#endif
    ...
    if(ADDR_GE((ptr_t)GC_least_plausible_heap_addr, (ptr_t)h) || EXPECT(NULL == GC_least_plausible_heap_addr, FALSE)) {
    ...
    if(ADDR(h) < GC_least_real_heap_addr || EXPECT(0 == GC_least_real_heap_addr, FALSE))
...
GC_INNER GC_bool
    ...
    GC_DBGLOG_PRINT_HEAP_IN_USE();
    if(GC_max_heapsize != 0 && (GC_max_heapsize < (word)sz || GC_heapsize > GC_max_heapsize - (word)sz)) {
    ...
    if((0 == GC_last_heap_addr && (ADDR(space) & SIGNB) == 0) || (GC_last_heap_addr != 0 && GC_last_heap_addr < ADDR(space))) {
...
GC_INNER GC_bool
    ...
    GC_ASSERT(GC_is_initialized);
    DISABLE_CANCEL(cancel_state);
    if(!GC_incremental && !GC_dont_gc && ((GC_dont_expand && GC_bytes_allocd > 0) || (GC_fo_entries > last_fo_entries && (last_bytes_finalized | GC_bytes_finalized) != 0 && (GC_fo_entries - last_fo_entries) * GC_allocd_bytes_per_finalizer > GC_bytes_allocd) || GC_should_collect())) {
    ...
    if(GC_max_heapsize > GC_heapsize) {
        ...
        if(blocks_to_get > max_get_blocks)
            blocks_to_get = max_get_blocks > needed_blocks ? max_get_blocks : needed_blocks;
    }
...
#define MAX_ALLOCOBJ_RETRIES 3
    ...
    while(NULL == *flh) {
        ...
        GC_ASSERT(!GC_is_full_gc || NULL == GC_obj_kinds[k].ok_reclaim_list || NULL == GC_obj_kinds[k].ok_reclaim_list[lg]);
