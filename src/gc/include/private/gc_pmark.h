...
GC_INLINE mse*
        GC_push_obj(ptr_t obj, const hdr* hhdr, mse* mark_stack_top, mse* mark_stack_limit) {
...
#define SET_MARK_BIT_EXIT_IF_SET(hhdr, bit_no)                                                \
    { /* cannot use do-while(0) here */                                                       \
        volatile unsigned char* mark_byte_addr = (unsigned char*)(hhdr)->hb_marks + (bit_no); \
...
#define TRACE_TARGET(target, cmd)                                                                 \
    if(GC_trace_ptr != NULL && GC_is_heap_ptr(GC_trace_ptr) && (target) == *(ptr_t*)GC_trace_ptr) \
    cmd
#else
#define TRACE(source, cmd)
...
GC_INLINE mse*
        GC_push_contents_hdr(ptr_t current, mse* mark_stack_top, mse* mark_stack_limit, ptr_t source, hdr* hhdr, GC_bool do_offset_check) {
    do {
...
#define GC_PUSH_ONE_STACK(p, source)                                                                                  \
    do {                                                                                                              \
        ptr_t pp = (p);                                                                                               \
                                                                                                                      \
        if(ADDR_LT((ptr_t)GC_least_plausible_heap_addr, p) && ADDR_LT(p, (ptr_t)GC_greatest_plausible_heap_addr)) {   \
            PUSH_ONE_CHECKED_STACK(p, source);                                                                        \
        }                                                                                                             \
        FIXUP_POINTER(pp);                                                                                            \
        if(ADDR_LT((ptr_t)GC_least_plausible_heap_addr, pp) && ADDR_LT(pp, (ptr_t)GC_greatest_plausible_heap_addr)) { \
