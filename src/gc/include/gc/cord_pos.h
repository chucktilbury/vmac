...
#define CORD_pos_fetch(p) \
    ((p)[0].cur_end != 0 ? (p)[0].cur_leaf[(p)[0].cur_pos - (p)[0].cur_start] : CORD__pos_fetch(p))

#define CORD_next(p) \
    ((p)[0].cur_pos + 1 < (p)[0].cur_end ? (p)[0].cur_pos++ : (CORD__next(p), 0U))

#define CORD_prev(p) \
    ((p)[0].cur_end != 0 && (p)[0].cur_pos > (p)[0].cur_start ? (p)[0].cur_pos-- : (CORD__prev(p), 0U))
