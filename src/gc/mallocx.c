...
#define cleared_p p
    ...
    if(sz > MAXOBJBYTES) {
        ...
        if(EXTRA_BYTES != 0 && (hhdr->hb_flags & IGNORE_OFF_PAGE) != 0 && obj_kind == NORMAL)
...
GC_API void GC_CALL
...
#ifdef PARALLEL_MARK
            if(GC_parallel) {
                GC_signed_word my_bytes_allocd_tmp = (GC_signed_word)AO_load(&GC_bytes_allocd_tmp);
    ...
    {
        struct hblk* h = GC_allochblk(lb_adjusted, k, 0 /* flags */, 0 /* align_m1 */);
...
#if !defined(wcsdup) && defined(REDIRECT_MALLOC) && !defined(REDIRECT_MALLOC_IN_HEADER)
wchar_t*
