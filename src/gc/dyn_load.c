...
#if defined(SOLARISDL) && defined(THREADS) && !defined(SOLARIS) && !defined(CPPCHECK)
...
#else
    stack_top = PTR_ALIGN_DOWN(GC_approx_sp(), GC_sysinfo.dwAllocationGranularity);
...
#if !(defined(CPPCHECK) || defined(AIX) || defined(DARWIN) || defined(DGUX) || defined(IRIX5) || defined(HAIKU) || defined(HPUX) || defined(HURD) || defined(NACL) || defined(OSF1) || defined(SCO_ELF) || defined(SERENITY) || defined(SOLARISDL) || ((defined(ANY_BSD) || defined(LINUX)) && defined(__ELF__)) || (defined(OPENBSD) && defined(M68K)))
#error Finding data segments of dynamic libraries is unsupported on target
#endif

#if defined(DARWIN) && !defined(USE_DYLD_TO_BIND) && !defined(NO_DYLD_BIND_FULLY_IMAGE)
#include <dlfcn.h>
#endif
...
#if defined(DGUX) || defined(HURD) || defined(NACL) || defined(SCO_ELF) || defined(SERENITY) || ((defined(ANY_BSD) || defined(LINUX)) && defined(__ELF__))
#include <stddef.h>
...
#include <link.h>
#endif /* HOST_ANDROID */
#if ((defined(HOST_ANDROID) && !defined(GC_DONT_DEFINE_LINK_MAP) && !(__ANDROID_API__ >= 21)) || defined(SERENITY)) && !defined(USE_PROC_FOR_LIBRARIES)
...
static void
    ...
    while(nsorted < n) {
        ...
        while(nsorted < n && ADDR_LT(base[nsorted - 1].hs_start, base[nsorted].hs_start)) {
            ++nsorted;
        }
        if(nsorted == n)
            break;
...
STATIC void
    ...
    for(;;) {
        maps = GC_parse_map_entry(maps, &my_start, &my_end, &prot, &maj_dev, &path);
        if(NULL == maps)
            break;
        ...
        if(prot[1] == 'w') {
            ...
            /* accounted for.                                   */
#ifndef THREADS
            if(ADDR_GE(GC_stackbottom, my_start) && ADDR_GE(my_end, GC_stackbottom)) {
            ...
            if(ADDR(my_start) == 0xc2fffffff000UL && ADDR(my_end) == 0xc30000000000UL && path[0] == '\n')
            ...
            for(; i < GC_n_memory && ADDR_LT(my_start, my_end) && ADDR_LT(GC_our_memory[i].hs_start, my_end);
                ++i) {
...
#if defined(CPPCHECK) && ((defined(HOST_ANDROID) && !defined(GC_DONT_DEFINE_LINK_MAP) && !(__ANDROID_API__ >= 21)) || defined(SERENITY))
...
#define MA_PHYS 0
    ...
    if(needed_sz >= current_sz) {
        ...
        addr_map = (prmap_t*)GC_scratch_alloc((size_t)current_sz * sizeof(prmap_t));
        if(NULL == addr_map)
    ...
    if(GC_n_heap_sects > 0) {
        heap_end = ADDR(GC_heap_sects[GC_n_heap_sects - 1].hs_start) + GC_heap_sects[GC_n_heap_sects - 1].hs_bytes;
        if(heap_end < GC_scratch_last_end_addr)
            heap_end = GC_scratch_last_end_addr;
    }
    for(i = 0; i < needed_sz; i++) {
        flags = addr_map[i].pr_mflags;
        if((flags & (MA_BREAK | MA_STACK | MA_PHYS | MA_FETCHOP | MA_NOTCACHED)) != 0)
            goto irrelevant;
        ...
        if(GC_roots_present(start) || (ADDR(start) >= heap_start && ADDR(start) < heap_end))
            goto irrelevant;
...
EXTERN_C_BEGIN
extern uint8_t* getsectiondata(const struct GC_MACH_HEADER*, const char* seg, const char* sect, unsigned long* psz);
EXTERN_C_END
#else
...
STATIC const struct dyld_sections_s {
    const char* seg;
    const char* sect;
} GC_dyld_sections[] = { { SEG_DATA, SECT_DATA },
...
STATIC const char* const GC_dyld_bss_prefixes[] = { "__bss", "__pu_bss", "__zo_bss", "__zo_pu_bss" };
...
#if !defined(USE_GETSECTBYNAME) && (MAC_OS_X_VERSION_MIN_REQUIRED < 1070 /*MAC_OS_X_VERSION_10_7*/)
#define USE_GETSECTBYNAME
#endif

static void
        dyld_section_add_del(const struct GC_MACH_HEADER* phdr, intptr_t slide, const char* dlpi_name, GC_has_static_roots_func callback, const char* seg, const char* secnam, GC_bool is_add) {
    ptr_t start, finish;
    ...
    if(is_add) {
        ...
        if(EXPECT(callback != 0, FALSE) && !callback(dlpi_name, start, (size_t)sec_size)) {
            UNLOCK();
            return; /* skip section */
        }
...
static void
        dyld_image_add_del(const struct GC_MACH_HEADER* phdr, intptr_t slide, GC_has_static_roots_func callback, GC_bool is_add) {
    unsigned i, j;
