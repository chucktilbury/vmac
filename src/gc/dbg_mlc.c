...
GC_INNER int
    ...
    if(((oh*)base)->oh_sf != (START_FLAG ^ (GC_uintptr_t)body) && ((GC_uintptr_t*)base)[BYTES_TO_PTRS(sz) - 1] != (END_FLAG ^ (GC_uintptr_t)body)) {
        return 0;
    }
...
GC_API GC_ref_kind GC_CALL
    ...
    /* insisting on even addresses.                                   */
    {
        ...
        if(GC_least_real_heap_addr < ADDR(alternate_target) && ADDR(alternate_target) < GC_greatest_real_heap_addr && (GC_least_real_heap_addr >= ADDR(target) || ADDR(target) >= GC_greatest_real_heap_addr)) {
            bp = alternate_ptr;
        }
    }
#endif
...
GC_INNER void*
        GC_store_debug_info_inner(void* base, size_t sz, const char* string, int linenum) {
    ...
    ((GC_uintptr_t*)base)[BYTES_TO_PTRS(GC_size(base)) - 1] = result[BYTES_TO_PTRS_ROUNDUP(sz)] = END_FLAG ^ (GC_uintptr_t)result;
#endif
    return result;
}
...
STATIC void
    ...
    if(ADDR_GE((ptr_t)(&ohdr->oh_sz), clobbered) || NULL == ohdr->oh_string) {
        ...
        GC_err_printf("%s %p in or near object at %p (%s:%d, sz= %lu)\n", msg,
                      (void*)clobbered, p,
                      ADDR(ohdr->oh_string) < HBLKSIZE ? "(smashed string)" : ohdr->oh_string[0] == '\0' ? "EMPTY(smashed?)" :
                                                                                                           ohdr->oh_string,
...
#ifdef GC_ADD_CALLER
#if defined(HAVE_DLADDR) && defined(GC_HAVE_RETURN_ADDR_PARENT) && defined(FUNCPTR_IS_DATAPTR)
#include <dlfcn.h>
...
GC_API GC_ATTR_MALLOC void* GC_CALL
    ...
    base = GC_malloc(lb < GC_SIZE_MAX - DEBUG_BYTES ? lb + DEBUG_BYTES : GC_SIZE_MAX >> 1);
#else
...
GC_API GC_ATTR_MALLOC void* GC_CALL
    ...
    void* base = GC_malloc_atomic_ignore_off_page(SIZET_SAT_ADD(lb, DEBUG_BYTES));
...
GC_API void GC_CALL
    ...
    *(void**)GC_is_visible(p) = GC_is_valid_displacement(GC_CAST_AWAY_CONST_PVOID(q));
...
GC_API GC_ATTR_MALLOC void* GC_CALL
    ...
    void* base = GC_malloc_uncollectable(SIZET_SAT_ADD(lb, UNCOLLECTABLE_DEBUG_BYTES));
...
GC_API void GC_CALL
    ...
    if(NULL == base) {
#if defined(REDIRECT_MALLOC) && ((defined(NEED_CALLINFO) && defined(GC_HAVE_BUILTIN_BACKTRACE)) || defined(REDIR_MALLOC_AND_LINUXTHREADS) || (defined(SOLARIS) && defined(THREADS)) || defined(MSWIN32))
...
GC_API void* GC_CALL
    ...
    hhdr = HDR(base);
    result = GC_debug_generic_or_special_malloc(lb, hhdr->hb_obj_kind, OPT_RA s, i);
    if(result != NULL) {
        size_t old_sz;
#ifdef SHORT_DBG_HDRS
...
static void
        store_old(void* obj, GC_finalization_proc my_old_fn, struct closure* my_old_cd, GC_finalization_proc* ofn, void** ocd) {
    if(my_old_fn != 0) {
...
GC_API void GC_CALL
        GC_debug_register_finalizer(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
...
GC_API void GC_CALL
        GC_debug_register_finalizer_no_order(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
    GC_finalization_proc my_old_fn = OFN_UNSET;
    void* my_old_cd = NULL;
    ptr_t base = (ptr_t)GC_base(obj);
    if(NULL == base) {
...
GC_API void GC_CALL
        GC_debug_register_finalizer_unreachable(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
    GC_finalization_proc my_old_fn = OFN_UNSET;
    void* my_old_cd = NULL;
    ptr_t base = (ptr_t)GC_base(obj);
    if(NULL == base) {
...
GC_API void GC_CALL
        GC_debug_register_finalizer_ignore_self(void* obj, GC_finalization_proc fn, void* cd, GC_finalization_proc* ofn, void** ocd) {
    GC_finalization_proc my_old_fn = OFN_UNSET;
    void* my_old_cd = NULL;
    ptr_t base = (ptr_t)GC_base(obj);
    if(NULL == base) {
