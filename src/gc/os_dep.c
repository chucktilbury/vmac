...
#if (defined(MPROTECT_VDB) && !defined(MSWIN32) && !defined(MSWINCE)) || (defined(SOLARIS) && defined(THREADS)) || defined(OPENBSD)
#include <signal.h>
#endif
...
#if defined(LINUX) && defined(SPECIFIC_MAIN_STACKBOTTOM) || defined(ADD_HEAP_GUARD_PAGES) || defined(MMAP_SUPPORTED) || defined(NEED_PROC_MAPS)
#include <errno.h>
#endif

#if defined(DARWIN) && !defined(DYNAMIC_LOADING) && !defined(GC_DONT_REGISTER_MAIN_STATIC_DATA)
...
#if ((defined(LINUX) && defined(SPECIFIC_MAIN_STACKBOTTOM) || defined(NEED_PROC_MAPS) || defined(PROC_VDB) || defined(SOFT_VDB)) && !defined(PROC_READ)) || defined(CPPCHECK)
/* Note: should probably call the real read, if read is wrapped.  */
#define PROC_READ read
#endif

#if defined(LINUX) && defined(SPECIFIC_MAIN_STACKBOTTOM) || defined(NEED_PROC_MAPS)
...
/* original buffer.                                                     */
#if defined(DYNAMIC_LOADING) && defined(USE_PROC_FOR_LIBRARIES) || defined(IA64) || defined(INCLUDE_LINUX_THREAD_DESCR) || (defined(CHECK_SOFT_VDB) && defined(MPROTECT_VDB)) || defined(REDIR_MALLOC_AND_LINUXTHREADS)
GC_INNER const char*
        GC_parse_map_entry(const char* maps_ptr, ptr_t* p_start, ptr_t* p_end, const char** p_prot, unsigned* p_maj_dev, const char** p_mapping_name) {
...
#if defined(IA64) || defined(INCLUDE_LINUX_THREAD_DESCR) || (defined(CHECK_SOFT_VDB) && defined(MPROTECT_VDB))
...
#define is_writable(prot) \
    ((prot) == PAGE_READWRITE || (prot) == PAGE_WRITECOPY || (prot) == PAGE_EXECUTE_READWRITE || (prot) == PAGE_EXECUTE_WRITECOPY)
...
GC_INNER void
...
#if defined(MSWINCE) && !defined(_WIN32_WCE_EMULATION)
    {
        ...
        if(verInfo.dwPlatformId == VER_PLATFORM_WIN32_CE && verInfo.dwMajorVersion < 6) {
...
#if defined(NEED_FIND_LIMIT) || (defined(UNIX_LIKE) && !defined(NO_DEBUGGING)) || (defined(USE_PROC_FOR_LIBRARIES) && defined(THREADS)) || (defined(WRAP_MARK_SOME) && defined(NO_SEH_AVAILABLE))
...
#if defined(NEED_FIND_LIMIT) || (defined(USE_PROC_FOR_LIBRARIES) && defined(THREADS)) || (defined(WRAP_MARK_SOME) && defined(NO_SEH_AVAILABLE))
...
STATIC ptr_t
    ...
    GC_ASSERT(up ? ADDR(bound) >= MIN_PAGE_SIZE : ADDR(bound) <= ~(word)MIN_PAGE_SIZE);
    GC_ASSERT(I_HOLD_LOCK());
...
#ifdef CHERI_PURECAP
    bound = (ptr_t)cheri_address_set(p, cheri_base_get(p) + (up ? cheri_length_get(p) : 0));
#else
...
#ifdef USE_LIBC_PRIVATES
    {
        ptr_t* p_libc_ia64_register_backing_store_base = &__libc_ia64_register_backing_store_base;
...
#endif
        if(p_libc_ia64_register_backing_store_base != NULL && __libc_ia64_register_backing_store_base != NULL) {
...
#elif !defined(ANY_MSWIN) && !defined(EMBOX) && !defined(OS2) && !(defined(OPENBSD) && defined(THREADS)) && !defined(SERENITY) && (!(defined(SOLARIS) && defined(THREADS)) || defined(_STRICT_STDC))
...
GC_INNER ptr_t
    ...
    if(pthread_attr_init(&attr) == 0 && (pthread_attr_get_np(pthread_self(), &attr) == 0 ? TRUE : (pthread_attr_destroy(&attr), FALSE)))
...
#define STACKBOTTOM_ALIGNMENT_M1 ((word)STACK_GRAN - 1)
...
#elif defined(HEURISTIC2)
    {
        ...
        if(HOTTER_THAN(HEURISTIC2_LIMIT, result) && HOTTER_THAN(sp, HEURISTIC2_LIMIT))
...
#if defined(DARWIN) && defined(THREADS) && !defined(NO_PTHREAD_GET_STACKADDR_NP)
#include <pthread.h>
...
typedef UINT(WINAPI* GetWriteWatch_type)(DWORD, PVOID, GC_ULONG_PTR /* SIZE_T */, PVOID*, GC_ULONG_PTR*, PULONG);
...
#endif
    if(hK32 != (HMODULE)0 && (GetWriteWatch_func = GetProcAddress(hK32, "GetWriteWatch")) != 0) {
        void* page;
...
STATIC void
    ...
    while(ADDR_LT(p, (ptr_t)GC_sysinfo.lpMaximumApplicationAddress)) {
        ...
        if(result != sizeof(buf) || 0 == buf.AllocationBase || GC_is_heap_base(buf.AllocationBase))
            break;
...
GC_INNER void*
    ...
    for(retry = 0;; retry++) {
        if(DosAllocMem(&result, bytes,
                       (PAG_READ | PAG_WRITE | PAG_COMMIT) | (GC_pages_executable ? PAG_EXECUTE : 0)) == NO_ERROR &&
...
GC_INNER void*
    ...
    for(i = 0; i < GC_n_heap_bases; i++) {
        if(((word)(-(GC_signed_word)GC_heap_lengths[i]) & (GC_sysinfo.dwAllocationGranularity - 1)) >= bytes) {
    ...
    if(i == GC_n_heap_bases) {
        /* Reserve more pages.  */
        size_t res_bytes = SIZET_SAT_ADD(bytes, (size_t)GC_sysinfo.dwAllocationGranularity - 1) & ~((size_t)GC_sysinfo.dwAllocationGranularity - 1);
...
#elif defined(AIX) || defined(COSMO) || defined(CYGWIN32) || defined(HPUX) || (defined(LINUX) && !defined(PREFER_MMAP_PROT_NONE))
...
#if (defined(COSMO) || defined(LINUX)) && !defined(FORCE_MPROTECT_BEFORE_MADVISE)
...
GC_INNER void
    ...
    while(len != 0) {
        ...
        result = (ptr_t)VirtualAlloc(start_addr, alloc_len, MEM_COMMIT,
                                     GC_pages_executable ? PAGE_EXECUTE_READWRITE : PAGE_READWRITE);
        if(result != start_addr) {
            if(GetLastError() == ERROR_NOT_ENOUGH_MEMORY || GetLastError() == ERROR_OUTOFMEMORY) {
...
#undef IGNORE_PAGES_EXECUTABLE
    ...
    {
#if !defined(SN_TARGET_PS3) && !defined(FORCE_MPROTECT_BEFORE_MADVISE) && (defined(LINUX) && !defined(PREFER_MMAP_PROT_NONE) || defined(COSMO))
...
#if defined(SOFT_VDB) && !defined(NO_SOFT_VDB_LINUX_VER_RUNTIME_CHECK) || (defined(GLIBC_2_19_TSX_BUG) && defined(GC_PTHREADS_PARAMARK))
GC_INNER int
    ...
    for(i = 0; i < GC_n_heap_sects; ++i) {
        ...
        do {
            ...
            if((*(GetWriteWatch_type)(GC_funcptr_uint)GetWriteWatch_func)(
                       ...
                       GC_heap_sects[i].hs_bytes, pages, &count, &page_size) != 0) {
...
#define PROTECT_INNER(addr, len, allow_write, C_msg_prefix)                                                                            \
    if(vm_protect(GC_task_self, (vm_address_t)(addr), (vm_size_t)(len),                                                                \
                  FALSE,                                                                                                               \
                  VM_PROT_READ | ((allow_write) ? VM_PROT_WRITE : 0) | (GC_pages_executable ? VM_PROT_EXECUTE : 0)) == KERN_SUCCESS) { \
    }                                                                                                                                  \
    else                                                                                                                               \
...
#endif
#if defined(AARCH64) || defined(ARM32) || defined(MIPS) || (__FreeBSD__ >= 7 || defined(OPENBSD))
...
#elif defined(AIX) || defined(COSMO) || defined(CYGWIN32) || defined(HAIKU) || defined(HURD) || defined(LINUX) || defined(NETBSD)
/* Linux: Empirically c.trapno == 14, on IA32, but is that useful?      */
/* Should probably consider alignment issues on other architectures.    */
#define CODE_OK TRUE
#elif defined(HPUX)
#define CODE_OK \
    (si->si_code == SEGV_ACCERR || si->si_code == BUS_ADRERR || si->si_code == BUS_UNKNOWN || si->si_code == SEGV_UNKNOWN || si->si_code == BUS_OBJERR)
...
GC_INNER GC_bool
    ...
    GC_ASSERT(GC_page_size != 0);
    if((GC_signed_word)(GC_heapsize / (word)GC_page_size) >= ((GC_signed_word)GC_UNMAPPED_REGIONS_SOFT_LIMIT - GC_num_unmapped_regions) * 2) {
...
#if defined(CAN_HANDLE_FORK) && defined(DARWIN) && defined(THREADS) || defined(COUNT_PROTECTED_REGIONS)
...
GC_INNER void
    ...
    if(GC_auto_incremental && !GC_GWW_AVAILABLE() && (GC_signed_word)(GC_heapsize / (word)GC_page_size) >= ((GC_signed_word)GC_UNMAPPED_REGIONS_SOFT_LIMIT - GC_num_unmapped_regions) * 2) {
...
static const pagemap_elem_t*
        pagemap_buffered_read(size_t* pres, off_t fpos, size_t len, off_t next_fpos_hint) {
    ssize_t res;
    size_t ofs;
    ...
    if(pagemap_buf_fpos <= fpos && fpos < pagemap_buf_fpos + (off_t)pagemap_buf_len) {
        ...
        off_t aligned_pos = fpos & ~(off_t)(GC_page_size < VDB_BUF_SZ ? GC_page_size - 1 : VDB_BUF_SZ - 1);
...
static void
        soft_set_grungy_pages(ptr_t start, ptr_t limit, ptr_t next_start_hint, GC_bool is_static_root) {
    ptr_t vaddr = (ptr_t)HBLK_PAGE_ALIGNED(start);
    off_t next_fpos_hint = (off_t)((ADDR(next_start_hint) >> GC_log_pagesize) * sizeof(pagemap_elem_t));
    ...
    while(ADDR_LT(vaddr, limit)) {
        ...
        for(; ADDR_LT(vaddr, limit_buf); vaddr += GC_page_size, bufp++) {
            if((*bufp & PM_SOFTDIRTY_MASK) != 0) {
...
#if defined(FILTER_PTRFREE_HBLKS_IN_SOFT_VDB) || defined(CHECKSUMS) || defined(DEBUG_DIRTY_BITS)
                    if(!is_static_root) {
                        hdr* hhdr;
                ...
                if(!is_static_root && get_pht_entry_from_index(GC_dirty_pages, PHT_HASH(vaddr))) {
...
GC_INLINE void
    ...
    if(getpid() != saved_proc_pid && (-1 == clear_refs_fd /* no need to retry */
                                      || (close(clear_refs_fd), close(pagemap_fd), !soft_dirty_open_files()))) {
...
#ifndef DONT_PROTECT_PTRFREE
        if(!is_ptrfree || !ADDR_INSIDE((ptr_t)current, (ptr_t)h, (ptr_t)(h + nblocks)))
#endif
        {
            async_set_pht_entry_from_index(GC_dirty_pages, index);
        }
...
extern kern_return_t exception_raise(mach_port_t, mach_port_t, mach_port_t, exception_type_t, exception_data_t, mach_msg_type_number_t);
...
GC_INNER void
    ...
    if(GC_old_exc_ports.count > 0) {
        ...
        if(task_set_exception_ports(
                   ...
                   GC_old_exc_ports.behaviors[0], GC_old_exc_ports.flavors[0]) != KERN_SUCCESS)
...
STATIC void*
    ...
    for(;;) {
        r = mach_msg(
                &msg.head,
                MACH_RCV_MSG | MACH_RCV_LARGE | (GC_mprotect_state == GC_MP_DISCARDING ? MACH_RCV_TIMEOUT : 0),
...
STATIC kern_return_t
        GC_forward_exception(mach_port_t thread, mach_port_t task, exception_type_t exception, exception_data_t data, mach_msg_type_number_t data_count) {
    size_t i;
    kern_return_t r;
    mach_port_t port;
...
GC_API_OSCALL kern_return_t
        catch_exception_raise(mach_port_t exception_port, mach_port_t thread, mach_port_t task, exception_type_t exception, exception_data_t code, mach_msg_type_number_t code_count) {
    kern_return_t r;
    char* addr;
...
GC_INNER void
    ...
    GC_ASSERT(I_HOLD_LOCK());
    info[0].ci_pc = CAST_THRU_UINTPTR(GC_return_addr_t, GC_save_callers_no_unlock);
...
GC_INNER void
    ...
    for(i = 0; i < NFRAMES; i++) {
...

        {
            char buf[40];
            char* name;
#if defined(GC_HAVE_BUILTIN_BACKTRACE) && !defined(GC_BACKTRACE_SYMBOLS_BROKEN) && defined(FUNCPTR_IS_DATAPTR)
...
#define PRELOAD_SZ 200
                ...
                if(old_preload != NULL && setenv("LD_PRELOAD", preload_buf, 0 /* overwrite */) == -1) {
                ...
                /* Get rid of embedded newline, if any.  Test for "main". */
                {
                    ...
                    if(strncmp(result_buf, "main",
                               nl != NULL ? (size_t)(ADDR(nl) /* a cppcheck workaround */
