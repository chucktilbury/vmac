...
#define GC_DECLARE_PTRFREE(T)        \
    ...
    template <>
    inline void*
            GC_selective_alloc<GC_true_type>(GC_ALCTR_SIZE_T n, GC_true_type, bool ignore_off_page) {
        void* obj = ignore_off_page ? GC_MALLOC_ATOMIC_IGNORE_OFF_PAGE(n) : GC_MALLOC_ATOMIC(n);
        if(0 == obj)
            GC_ALLOCATOR_THROW_OR_ABORT();
        return obj;
    }
