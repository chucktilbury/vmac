...
#if (GC_GNUC_PREREQ(3, 0) || defined(__clang__)) && !defined(GC_NO_PREFETCH_FOR_WRITE)
#define GC_PREFETCH_FOR_WRITE(x) __builtin_prefetch((x), 1 /* write */)
#elif defined(_MSC_VER) && !defined(GC_NO_PREFETCH_FOR_WRITE) && (defined(_M_IX86) || defined(_M_X64)) && !defined(_CHPE_ONLY_) && (_MSC_VER >= 1900) /* VS 2015+ */
#include <intrin.h>
    ...
    do {                                                                                                                                 \
        if(GC_EXPECT((lg) >= GC_TINY_FREELISTS, 0)) {                                                                                    \
            ...
            void* next;                                                                                                                  \
                                                                                                                                         \
            for(;;) {                                                                                                                    \
                if(GC_EXPECT((GC_word)(GC_uintptr_t)my_entry > (num_direct) + GC_TINY_FREELISTS + 1,                                     \
                             1)) {                                                                                                       \
                    next = *(void**)(my_entry);                                                                                          \
                    result = my_entry;                                                                                                   \
                    ...
                    GC_ASSERT((k) == GC_I_PTRFREE || 0 /* NULL */ == ((void**)result)[1]);                                               \
                    break;                                                                                                               \
                }                                                                                                                        \
                /* Entry contains counter or NULL. */                                                                                    \
                if((GC_signed_word)(GC_uintptr_t)my_entry - (GC_signed_word)(num_direct) <= 0 /* (GC_uintptr_t)my_entry <= num_direct */ \
                   && my_entry != 0 /* NULL */) {                                                                                        \
