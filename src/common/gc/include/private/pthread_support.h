...
#define DETACHED 0x2
#endif
#if (defined(GC_HAVE_PTHREAD_EXIT) || !defined(GC_NO_PTHREAD_CANCEL)) && defined(GC_PTHREADS)
...
#if defined(GC_WIN32_THREADS) && !defined(CYGWIN32) && (defined(GC_WIN32_PTHREADS) || defined(GC_PTHREADS_PARAMARK)) && !defined(__WINPTHREADS_VERSION_MAJOR)
...
#define THREAD_TABLE_INDEX(id) \
    (int)(((((NUMERIC_THREAD_ID(id) >> 8) ^ NUMERIC_THREAD_ID(id)) >> 16) ^ ((NUMERIC_THREAD_ID(id) >> 8) ^ NUMERIC_THREAD_ID(id))) % THREAD_TABLE_SZ)
#else
#define THREAD_TABLE_INDEX(id) \
    (int)(((NUMERIC_THREAD_ID(id) >> 16) ^ (NUMERIC_THREAD_ID(id) >> 8) ^ NUMERIC_THREAD_ID(id)) % THREAD_TABLE_SZ)
#endif
