...
#if (!defined(AO_HAVE_test_and_set_acquire) || defined(GC_WIN32_THREADS) || defined(LINT2) || defined(RTEMS) || defined(SN_TARGET_PS3) || defined(BASE_ATOMIC_OPS_EMULATED) || defined(USE_RWLOCK)) && defined(GC_PTHREADS)
#define USE_PTHREAD_LOCKS
#undef USE_SPIN_LOCK
#if (defined(GC_WIN32_THREADS) || defined(LINT2) || defined(USE_RWLOCK)) && !defined(NO_PTHREAD_TRYLOCK)
...
#elif (!defined(THREAD_LOCAL_ALLOC) || defined(USE_SPIN_LOCK)) && !defined(USE_PTHREAD_LOCKS) && !defined(THREAD_SANITIZER) && !defined(USE_RWLOCK)
...
#define set_need_to_lock() \
    (void)(*(GC_bool volatile*)&GC_need_to_lock ? FALSE : (GC_need_to_lock = TRUE))
#else
