...
void CORD_set_oom_fn(CORD_oom_fn_t fn) {
    CORD_oom_fn = fn;
}
...
#define LEFT_LEN(s) \
    (((const CordRep*)s)->generic.left_len != 0 ? ((const CordRep*)s)->generic.left_len : (CORD_IS_STRING(((const CordRep*)s)->data.concat.left) ? ((const CordRep*)s)->generic.len - GEN_LEN(((const CordRep*)s)->data.concat.right) : LEN(((const CordRep*)s)->data.concat.left)))
...
CORD CORD_cat_char_star(CORD x, const char* y, size_t leny) {
    ...
    if(CORD_IS_STRING(x)) {
        ...
        if(leny <= SHORT_LIMIT / 2 && IS_CONCATENATION(x) && CORD_IS_STRING(right = ((const CordRep*)x)->data.concat.right)) {
            size_t right_len;
...
static void
    ...
    for(i = 2; i < CORD_MAX_DEPTH; i++) {
        size_t current = last < (~(size_t)0) - previous ? last + previous : last /* overflow */;
...
static void
    ...
    else if(IS_CONCATENATION(x) && ((depth = DEPTH(x)) >= CORD_MAX_DEPTH || len < min_len[depth])) {
        const struct Concatenation* conc = &((const CordRep*)x)->data.concat;
        size_t left_len = LEFT_LEN(x);
...
void CORD__next(CORD_pos p) {
    ...
    {
        while(p[0].path_len > 0 && current_pe[0].pe_start_pos != current_pe[-1].pe_start_pos) {
            p[0].path_len--;
            current_pe--;
        }
        if(p[0].path_len == 0) {
