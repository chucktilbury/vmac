...
static int
        extract_conv_spec(CORD_pos source, char* buf, int* width, int* prec, int* left, int* long_arg) {
    int result = 0;
                ...
                /* Use standard sprintf to perform conversion */
                {
...
#elif defined(__GNUC__) && !defined(__DJGPP__) && !defined(__EMX__) /* and probably in other cases */
                    va_copy(vsprintf_args, args);
#else
                    vsprintf_args = args;
#endif
                    ...
                    if(0 == res)
                        res = GC_VSNPRINTF(buf, (unsigned)max_size + 1, conv_spec,
                                           vsprintf_args);
#if defined(CPPCHECK) || defined(__va_copy) || (defined(__GNUC__) && !defined(__DJGPP__) && !defined(__EMX__))
