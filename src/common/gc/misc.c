...
#elif defined(GC_DEFN_ALLOCATE_ML) && !defined(USE_RWLOCK) || defined(SN_TARGET_PS3)
#include <pthread.h>
...
GC_finalizer_notifier_proc GC_finalizer_notifier = (GC_finalizer_notifier_proc)0;
...
void* GC_clear_stack_inner(void* arg, CLEARSTACK_LIMIT_MODIFIER ptr_t limit) {
...
GC_API void* GC_CALL
    ...
    r -= HBLKDISPL(r) % sz;
    limit = r + sz;
    if((ADDR_LT((ptr_t)(h + 1), limit) && sz <= HBLKSIZE) || ADDR_GE((ptr_t)p, limit))
        return NULL;
...
GC_API void GC_CALL
        GC_get_heap_usage_safe(GC_word* pheap_size, GC_word* pfree_bytes, GC_word* punmapped_bytes, GC_word* pbytes_since_gc, GC_word* ptotal_bytes) {
    READER_LOCK();
    if(pheap_size != NULL)
...
#if !defined(OS2) && !defined(GC_ANDROID_LOG) && !defined(NN_PLATFORM_CTR) && !defined(NINTENDO_SWITCH) && (!defined(MSWIN32) || defined(CONSOLE_LOG)) && !defined(MSWINCE)
...
#define SPIN_COUNT 4000
    ...
    if(EXTRA_BYTES != 0)
        GC_obj_kinds[NORMAL].ok_descriptor = ((~(word)ALIGNMENT) + 1) | GC_DS_LENGTH;
#endif
...
static GC_bool
    ...
    static const GUID kIID_IApplicationDataStatics = { 0x5612147B, 0xE843, 0x45E3, 0x94, 0xD8, 0x06,
                                                       0x16, 0x9E, 0x3C, 0x8E, 0x17 };
    static const GUID kIID_IStorageItem = { 0x4207A996, 0xCA2F, 0x42F7, 0xBD, 0xE8, 0x8B,
       ...
       SUCCEEDED(RoGetActivationFactory(
        ...
        if(SUCCEEDED(
           ...
           SUCCEEDED(
            ...
            UINT32 tempPathLen;
            const wchar_t* tempPathBuf = WindowsGetStringRawBuffer(tempPath, &tempPathLen);
...
STATIC HANDLE
    ...
    if(getWinRTLogPath(pathBuf, _MAX_PATH + 1)) {
        ...
        extParams.dwFileFlags = GC_print_stats == VERBOSE ? 0 : FILE_FLAG_WRITE_THROUGH;
...
#define appendToFile FALSE
    ...
    if(GetEnvironmentVariable(TEXT("GC_LOG_FILE"), pathBuf, _MAX_PATH + 1) - 1U < (DWORD)_MAX_PATH) {
        appendToFile = TRUE;
    }
    else
#endif
...
#else
        int len = (int)GetModuleFileName(NULL /* hModule */, pathBuf, _MAX_PATH + 1);
...
#if defined(_MSC_VER) && defined(_DEBUG) && !defined(NO_CRT) && !defined(NO_CRTDBGREPORT)
#ifdef MSWINCE
...
#if !defined(ECOS) && !defined(NOSYS) && !defined(PLATFORM_WRITE) && !defined(SN_TARGET_PSP2)
#include <errno.h>
#endif
