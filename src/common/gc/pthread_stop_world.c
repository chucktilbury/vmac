...
STATIC void
    ...
    if(sigdelset(set, SIGINT) != 0 || sigdelset(set, SIGQUIT) != 0 || sigdelset(set, SIGABRT) != 0 || sigdelset(set, SIGTERM) != 0) {
...
#elif defined(HPUX) || defined(NETBSD) || defined(OSF1) || defined(GC_USESIGRT_SIGNALS)
...
GC_API int GC_CALL
    ...
    return GC_sig_thr_restart != SIGNAL_UNSET ? GC_sig_thr_restart : SIG_THR_RESTART;
}
...
#ifdef GC_ENABLE_SUSPEND_THREAD
            || ((suspend_cnt & 1) != 0 && ao_load_async(&me->ext_suspend_cnt) == suspend_cnt)
#endif
    );
...
#define TS_NSEC_ADD(ts, ns) \
    (ts.tv_nsec += (ns),    \
     (void)(ts.tv_nsec >= 1000000L * 1000 ? (ts.tv_nsec -= 1000000L * 1000, ts.tv_sec++, 0) : 0))
#endif
    ...
    for(i = 0; i < THREAD_TABLE_SZ; i++) {
        for(p = GC_threads[i]; p != NULL; p = p->tm.next) {
            ...
            if(THREAD_EQUAL(p->id, self)) {
                ...
                bs_hi = crtn->backing_store_ptr;
#endif
                if(traced_stack_sect != NULL && traced_stack_sect->saved_stack_ptr == lo) {
            ...
            if(crtn->altstack != NULL && ADDR_GE(lo, crtn->altstack) && ADDR_GE(crtn->altstack + crtn->altstack_size, lo)) {
#ifdef STACK_GROWS_UP
                hi = crtn->altstack;
#else
...
EXTERN_C_BEGIN
extern size_t nacl_interface_query(const char* interface_ident, void* table, size_t tablesize);
EXTERN_C_END
        ...
        for(p = GC_threads[i]; p != NULL; p = p->tm.next) {
            if(!THREAD_EQUAL(p->id, self)) {
                ...
                if((p->ext_suspend_cnt & 1) != 0)
                    continue;
#endif
                if(GC_retry_signals && AO_load(&p->last_stop_count) == GC_stop_count) {
