...
STATIC size_t
    ...
    if(blocks_needed >= HUGE_THRESHOLD)
        return N_HBLK_FLS;
    return (blocks_needed - UNIQUE_THRESHOLD) / FL_COMPRESSION + UNIQUE_THRESHOLD;
}
...
static void GC_CALLBACK
    ...
    GC_printf("\t%p size %lu %s black listed\n", (void*)h,
              ...
              GC_is_black_listed(h, HBLKSIZE) != NULL ? "start" : GC_is_black_listed(h, hhdr->hb_sz) != NULL ? "partially" :
                                                                                                               "not");
#endif
}
...
GC_API void GC_CALL
    ...
    for(i = 0; i < GC_n_heap_sects; ++i) {
        ...
        while(i + 1 < GC_n_heap_sects && GC_heap_sects[i + 1].hs_start == finish) {
            ++i;
        ...
        for(p = start; ADDR_LT(p, finish);) {
            ...
            if(HBLK_IS_FREE(hhdr)) {
                int correct_index = (int)GC_hblk_fl_from_blocks(divHBLKSZ(hhdr->hb_sz));
                int actual_index;
...
static GC_bool
        setup_header(hdr* hhdr, struct hblk* block, size_t lb_adjusted, int k, unsigned flags) {
    ...
    if(EXTRA_BYTES != 0 && (flags & IGNORE_OFF_PAGE) != 0 && k == NORMAL && lb_adjusted >= HBLKSIZE)
...
STATIC void
...
#if defined(GC_ASSERTIONS) && !defined(USE_MUNMAP) && !defined(CHERI_PURECAP)
    {
        ...
        GC_ASSERT(NULL == nexthdr || !HBLK_IS_FREE(nexthdr) || (GC_heapsize & SIGNB) != 0);
...
GC_INNER GC_bool
    ...
    for(i = 0; i <= N_HBLK_FLS; ++i) {
        ...
        while(h != NULL) {
            ...
            if(NULL == nexthdr || !HBLK_IS_FREE(nexthdr) || BLOCKS_MERGE_OVERFLOW(hhdr, nexthdr)) {
...
STATIC void
        GC_split_block(struct hblk* hbp, hdr* hhdr, struct hblk* last_hbp, hdr* last_hdr, size_t index /* of free list */) {
...
STATIC struct hblk* GC_allochblk_nth(size_t lb_adjusted, int k, unsigned flags, size_t index, int may_split, size_t align_m1);
...
GC_INNER struct hblk*
        GC_allochblk(size_t lb_adjusted, int k, unsigned flags /* IGNORE_OFF_PAGE or 0 */, size_t align_m1) {
    ...
    result = GC_allochblk_nth(lb_adjusted, k, flags, start_list, FALSE, align_m1);
    if(result != NULL)
        return result;
...
static GC_bool
        next_hblk_fits_better(const hdr* hhdr, size_t size_avail, size_t size_needed, size_t align_m1) {
    const hdr* nexthdr;
    size_t next_size;
    size_t next_ofs;
...
static struct hblk*
        find_nonbl_hblk(struct hblk* last_hbp, size_t size_remain, size_t eff_size_needed, size_t align_m1) {
#ifdef NO_BLACK_LISTING
    UNUSED_ARG(size_remain);
    UNUSED_ARG(eff_size_needed);
    return last_hbp + divHBLKSZ(ALIGN_PAD_SZ(last_hbp, align_m1));
#else
    ptr_t search_end = PTR_ALIGN_DOWN((ptr_t)last_hbp + size_remain, align_m1 + 1);
...
STATIC struct hblk*
        GC_allochblk_nth(size_t lb_adjusted, int k, unsigned flags, size_t index, int may_split, size_t align_m1) {
...
retry:
    ...
    for(hbp = GC_hblkfreelist[index];; hbp = hhdr->hb_next) {
        ...
        if(GC_page_size != HBLKSIZE && (!GC_incremental /* not enabled yet */
                                        || GC_incremental_protection_needs() != GC_PROTECTS_NONE) &&
           is_hblks_mix_in_page(hbp, k == PTRFREE))
            continue;
#endif

        if(IS_UNCOLLECTABLE(k) || (k == PTRFREE && size_needed <= MAX_BLACK_LIST_ALLOC)) {
