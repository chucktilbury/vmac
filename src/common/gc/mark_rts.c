...
#endif
    if(last_static_root_set < n_root_sets && ADDR_INSIDE(p, GC_static_roots[last_static_root_set].r_start, GC_static_roots[last_static_root_set].r_end))
        return TRUE;
    for(i = 0; i < n_root_sets; i++) {
...
STATIC void
    ...
    GC_log_printf("Remove data root section at %u: %p .. %p%s\n", (unsigned)i,
                  ...
                  GC_static_roots[i].r_tmp ? " (temporary)" : "");
#endif
    GC_root_size -= (word)(GC_static_roots[i].r_end - GC_static_roots[i].r_start);
...
STATIC void
    ...
    GC_ASSERT(I_HOLD_LOCK());
    for(i = 0; i < n_root_sets;) {
        if(ADDR_GE(GC_static_roots[i].r_start, b) && ADDR_GE(e, GC_static_roots[i].r_end)) {
            GC_remove_root_at_pos(i);
        }
        else {
            i++;
        ...
        if(!EXPECT(ADDR_GE(r_start, e) || ADDR_GE(b, r_end), TRUE)) {
            ...
            if(ADDR_LT(r_start, b)) {
                ...
                if(ADDR_LT(e, r_end)) {
                    ...
                    GC_remove_root_at_pos(i);
                    if(i + 1 < n_root_sets && GC_static_roots[i].r_tmp && !GC_static_roots[i + 1].r_tmp) {
                        size_t j;
...
GC_API int GC_CALL
    ...
    i = last_root_set;
#endif
    if(i < n_root_sets && ADDR_INSIDE((ptr_t)p, GC_static_roots[i].r_start, GC_static_roots[i].r_end)) {
...
GC_API void GC_CALL
    ...
    e = EXPECT(ADDR(e) > ~(word)(ALIGNMENT - 1), FALSE) ? PTR_ALIGN_DOWN((ptr_t)e, ALIGNMENT) /* overflow */
                                                          :
...
#define GC_PUSH_CONDITIONAL(b, t, all) \
    (GC_parallel ? GC_push_conditional_eager(b, t, all) : GC_push_conditional_static(b, t, all))
#else
...
GC_INNER void
        GC_push_all_register_sections(ptr_t bs_lo, ptr_t bs_hi, GC_bool eager, struct GC_traced_stack_sect_s* traced_stack_sect) {
    GC_ASSERT(I_HOLD_LOCK());
...
GC_INNER void
        GC_push_all_stack_sections(ptr_t lo /* top */, ptr_t hi /* bottom */, struct GC_traced_stack_sect_s* traced_stack_sect) {
    GC_ASSERT(I_HOLD_LOCK());
    while(traced_stack_sect != NULL) {
...
STATIC void
    ...
    GC_ASSERT(traced_stack_sect == NULL || cold_gc_frame == NULL || HOTTER_THAN(cold_gc_frame, (ptr_t)traced_stack_sect));
...
STATIC void
    ...
    /* GC_push_all_stack_partially_eager.                       */
    {
        ...
        if(GC_all_interior_pointers && ADDR_LT(GC_register_stackbottom, cold_gc_bs_pointer)) {
            /* Adjust cold_gc_bs_pointer if below our innermost   */
            /* "traced stack section" in backing store.           */
            if(GC_traced_stack_sect != NULL && ADDR_LT(cold_gc_bs_pointer, GC_traced_stack_sect->backing_store_end)) {
